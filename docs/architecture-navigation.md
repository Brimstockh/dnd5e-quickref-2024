...
Les surfaces illustrées utilisent une palette locale claire (`--visual-text`, `--visual-text-muted`, `--visual-text-subtle`) indépendante du thème global. `section-visual` sert aux cartes de sections et aux hubs ; `page-feature` conserve son layout éditorial propre, mais partage ces tokens, les positions d’artwork et trois intentions d’overlay (`hero`, `card`, `compact`). La hiérarchie visuelle est : espace immersif, catalogue/famille avec `page-feature--compact`, puis entrée individuelle avec illustration spécifique si disponible.

Le contrat est déployé sur les pages de niveau 2 de Création et du Compendium. Classes réutilise `classes-heroes.webp` comme hero compact spécialisé de Création ; Espèces, Historiques et Dons utilisent `page-feature--creation` avec un fond atmosphérique CSS générique ; Sorts, Monstres, Objets magiques, Armes et armures, Matériel d’aventurier et Services, montures et véhicules utilisent `page-feature--compendium`, avec leurs icônes respectives et sans nouvelle illustration. Les recherches, filtres, tableaux, résultats et chargements progressifs restent indépendants du hero.

Le contrat final combine une classe de section (`page-feature--classes`, `page-feature--creation` ou `page-feature--compendium`) avec `page-feature--compact`. L’accent vient de l’espace ; le titre, l’icône et l’éventuel artwork portent l’identité du contenu. `page-feature--classes` est l’exception illustrée avec `classes-heroes.webp`, tandis que les autres pages Création utilisent pour l’instant le fond atmosphérique générique. `section-visual` reste réservé aux hubs de niveau 1 ; il ne doit pas être confondu avec `page-feature--compact`.
